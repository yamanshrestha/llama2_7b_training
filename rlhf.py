...
base_model = "meta-llama/Meta-Llama-3-8B" #, use_auth_token="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"
new_model = sys.argv[2] #give a new model name
...
training_arguments = TrainingArguments(
        output_dir="./results", # Directory where the checkpoints will be saved
        num_train_epochs=4, #4  Total number of training epochs
        per_device_train_batch_size=4, #10 # Batch size per GPU
        gradient_accumulation_steps=5, #1 Number of updates steps to accumulate before performing a backward/update pass.
        gradient_checkpointing= True, # Enable gradient checkpointing to save memory
        evaluation_strategy="steps",
        eval_steps=100, # Evaluation and saving checkpoints every 1000 steps
        logging_steps=1,
        save_strategy="steps",  # Save strategy 
        save_steps=100,     #Save a checkpoint every 1000 steps
